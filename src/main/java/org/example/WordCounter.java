...
public static Map<String, Integer> calculatedWord (){
    ...
    String[] splitText = text.replaceAll("[^\\p{L}'-]", " ").toLowerCase().split(" ");

    for (String word: splitText ){
        if(calculate.containsKey(word)){
            calculate.put(word, calculate.get(word)+1);
        } else {
            calculate.put(word, 1);
        }
    }
    return calculate;
}
}
// collection frameworks (I's) , map(I) , collections(c)